"""Ref: https://github.com/pseulki/IB-Loss/"""
import numpy as np
import torch
...
class IBLoss(nn.Module):
    def __init__(self, num_classes, weight=None, alpha=10000.0):
    ...
    def forward(self, input, target, features):
        grads = torch.sum(
            torch.abs(
                F.softmax(input, dim=1) - F.one_hot(target, self.num_classes)
            ),
            1,
        )  # N * 1
...
class IB_FocalLoss(nn.Module):
    def __init__(self, num_classes, weight=None, alpha=10000.0, gamma=0.0):
