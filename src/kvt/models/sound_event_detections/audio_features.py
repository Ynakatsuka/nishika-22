...
def compute_deltas(
    ...
    Args:
        ...
        win_length (int, optional): The window length used for computing delta
            (Default: ``5``)
        mode (str, optional): Mode parameter passed to padding
            (Default: ``"replicate"``)
...
def pcen(
    ...
    if training:
        ...
        pcen_ = (
            x.div_(M.add_(eps).pow_(alpha)).add_(delta).pow_(r).sub_(delta ** r)
        )
    return pcen_


class PCENTransform(nn.Module):
    """Ref: https://bit.ly/2Z92WD1"""
